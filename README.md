...
2. The app initializes IndexedDB and (optionally) loads sample data.
3. Create a Schedule (draft is fine; application number can be added after approval).
4. Add Series Items by selecting an existing schedule from the UI (don't free-type schedule numbers).
5. Use filters/search to find items; click a result to view/edit details.
...
1. Click **Export JSON**.
2. A file like `iletsb-records-backup-YYYY-MM-DD.json` downloads.
3. It includes schedules, series_items, and audit_events.
...
1. Click **Import JSON** and choose a valid backup.
2. The app validates the file and shows a summary.
...
2. IndexedDB → ILETSBRecords → Delete database
3. Refresh the page
